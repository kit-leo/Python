...
involved_parents = set()
def search(clazz, clazz_p):
    if clazz_p in involved_parents:
        return True
    elif len(dict_main[clazz_p]) > 0:
        for parent in dict_main[clazz_p]:
            if search(clazz, parent):
                return True
    else:
